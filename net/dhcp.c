...
int dhcp_configure(void) {
    ...
    *o++ = 255;
    if (net_send_udp(IP_BROADCAST, 68, 67, &m, DHCP_MSG_LEN) != 0)
        return 0;

    if (!wait_reply(xid, 2, &reply, 4000))         /* OFFER */
        return 0;
    ...
    if (sid && vl == 4) memcpy(server_id, sid, 4);
    ...
    *o++ = 50; *o++ = 4; ip_to_bytes(offered, o); o += 4;   /* requested IP */
    if (sid && vl == 4) { *o++ = 54; *o++ = 4; memcpy(o, server_id, 4); o += 4; }
    *o++ = 55; *o++ = 3; *o++ = 1; *o++ = 3; *o++ = 6;
    *o++ = 255;
    if (net_send_udp(IP_BROADCAST, 68, 67, &m, DHCP_MSG_LEN) != 0)
        return 0;

    if (!wait_reply(xid, 5, &reply, 4000))         /* ACK */
        return 0;
